...
<?php
		...
		if (isset($_POST['submit'])) {
			...
			$required_fields = array('survey_menu_name', 'position', 'visible');
			foreach($required_fields as $fieldname) {
				if (!isset($_POST[$fieldname]) || (empty($_POST[$fieldname]) && $_POST[$fieldname] != 0)) { 
					$errors[] = $fieldname; 
				}
			}
			$fields_with_lengths = array('survey_menu_name' => 30);
			...
			if (empty($errors)) {
				...
				$menu_name = mysql_prep($_POST['survey_menu_name']);
				$position = mysql_prep($_POST['position']);
				$visible = mysql_prep($_POST['visible']);
				
				$query = "UPDATE survey_subject SET 
							survey_menu_name = '{$menu_name}', 
							position = {$position}, 
							visible = {$visible} 
						WHERE id = {$id}";
...
<table id="structure">
	<tr>
		...
		<td id="page">
			<h2>Edit Subject: <?php echo $sel_subject['survey_menu_name']; ?></h2>
			...
			<form action="edit_subject.php?subj=<?php echo urlencode($sel_subject['id']); ?>" method="post">
				<p>Subject name: 
					<input type="text" name="survey_menu_name" value="<?php echo $sel_subject['survey_menu_name']; ?>" id="survey_menu_name" />
				</p>
				<p>Position: 
